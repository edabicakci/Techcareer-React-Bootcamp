...
// *CustomerId,shipName, city, orderDate (yıl-ay şeklinde yani 1997-12 gibi) ve order'a ait TotalPrice (derste hesapladığımız 
//gibi detaydaki ürünlerin quantity*price şeklinde toplamları) şeklinde 5 kolonlu bir tablo oluşuturulacak.
...
//     if (order.shipVia === 1){
//         ulElement.innerHTML += `<li style= "color: red;">${customerId}</li>`
//     }
//     else if(order.shipVia === 2){
//         ulElement.innerHTML += `<li class= "bolded">${customerId}</li>`
//     }
//     else{
...
orders.forEach(order => {
    ...
    let customerId = order.customerId;
    let shipName = order.shipName;
    let city = order.shipAddress?.city;
    let orderDate = order.orderDate;
    orderDate = new Date (orderDate);
    newDate = orderDate.getFullYear() + "-" + (orderDate.getMonth() + 1);
    let totalPrice = calculateTotalPrice(order).toFixed(2);
